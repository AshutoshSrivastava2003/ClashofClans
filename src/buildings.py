...
class Cannon(Building):
    ...
    def scan_for_targets(self, King):
        ...
        specialtroops= stealtharchers
        for troop in specialtroops:
            if(time.time()-troop.init_time>10):
                troop.invisible=0
                if (troop.position[0] - self.position[0])**2 + (troop.position[1] - self.position[1])**2 <= self.attack_radius**2:
                    self.isShooting = True
                    self.attack_target(troop)
...
class Wall(Building):
    ...
    def explode(self):
        ...
        for troop in troops:
        # for troop in self.V.get_ground_troops():
            if (abs(troop.position[0] - self.position[0]) <= 2 and
                    abs(troop.position[1] - self.position[1]) <= 2):
