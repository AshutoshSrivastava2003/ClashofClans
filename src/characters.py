...
def clearTroops():
    ...
    troops_spawned['stealth archer'] = 0
...
class Healer():
    ...
    def move(self, troop, V):
        ...
        if(r*r+c*c<self.range**2):
            ...
            for troop2 in troops:
                if troop.alive==False:
                    continue
                # dist=+abs(troop.position[1] - pos[1])
                if (abs(troop2.position[0] - pos[0])<=1 and abs(troop2.position[1] - pos[1])<=1 ):
                    troop2.health+=self.heal_strength
                    troop2.health%=troop2.max_health
...
def spawnStealthArcher(pos):
    if(pt.troop_limit['stealth archer'] <= troops_spawned['stealth archer']):
        return

    # convert tuple to list
    pos = list(pos)
    StArcher = StealthArcher(pos,time.time())
    troops_spawned['stealth archer'] += 1
    stealtharchers.append(StArcher)
...
def move_healers(V):
    for heal in healers:
        if(heal.alive == False):
            continue
        # closest_building = search_for_closest_building(heal.position, V.map, 1)
        closest_troop = search_for_closest_troop(heal.position,V.map,1)
        if(closest_troop == None):
            continue
        # else:
        #     print(closest_troop.health)
