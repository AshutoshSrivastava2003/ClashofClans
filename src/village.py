...
class Village:
    def __init__(self, config, level):
        ...
        self.map = self.generate_map(level)             # map of the village
    ...
    def generate_map(self,level):
        ...
        for wall in self.walls_top:                                     # wall is 1x1
            wall_obj = bd.Wall(wall,2*level-1, self)
            # wall_obj.level=2*level-1
        ...
        for cannon in self.cannons:                                     # cannon is 2x2
            cannon_obj = bd.Cannon(cannon, 2*level-1,self)
        ...
        for wizard_tower in self.wizard_towers:                          # wizard tower is 1X1
            wizard_tower_obj = bd.WizardTower(wizard_tower,2*level-1, self)
